...
console.log("refund coupon",req.body);
 ...
 if(req.body.code == "Bank"){
  ...
  descri: "A Coupon with the same value of the amount paid which can used for any services in Petfolio App. Coupon value of Rs."+req.body.amount+"/-",
  active_status : true,
  delete_status : false,
        }, 
        function (err, user) {
          console.log("refund Coupon 1",user);
           console.log("refund Coupon err",err);
        res.json({Status:"Success",Message:"Ref Code Added successfully", Data : user ,Code:200}); 
        });
}
catch(e){
   console.log("refund Coupon err 2",e);
