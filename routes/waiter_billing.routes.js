...
var waiter_billingModel = require('./../models/waiter_billingModel');
// var VerifyToken = require('./VerifyToken');
// const { check, validationResult } = require('express-validator');

router.post('/create', async function(req, res) {

  console.log("billing_details",req.body);
  try{
     await waiter_billingModel.create({
...
await waiter_billingModel.find({
    createdAt: {
        $gte: st_date,
        $lte:  en_date
    },
...
await waiter_billingModel.find({rest_id : req.body.rest_id}, function (err, UserDetails) {
      if (err) return res.status(500).send("There was a problem deleting the user.");
...
router.get('/getlist', async function (req, res) {
        await waiter_billingModel.find({}, function (err, CategoryDetails) {
...
router.get('/deletes', function (req, res) {
      waiter_billingModel.remove({}, function (err, user) {
      if (err) return res.status(500).send("There was a problem deleting the user.");
...
router.get('/getlist_dropdown', async function (req, res) {
        await waiter_billingModel.find({status:'false'}, function (err, CategoryDetails) {
          if(err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
           if(CategoryDetails == ""){
            return res.json({Status:"Failed",Message:"No data Found", Data : {},Code:404});
           }
...
router.post('/edit', async function (req, res) {
  console.log(req.body);
        await waiter_billingModel.findByIdAndUpdate(req.body._id, req.body, {new: true}, function (err, UpdatedDetails) {
            if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
             if(UpdatedDetails == ""){
            return res.json({Status:"Failed",Message:"No data Found", Data : {},Code:404});
           }
...
for(let a = 0 ; a < req.body.goals.length ; a ++){
        await waiter_billingModel.findByIdAndRemove(req.body.goals[a], function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
           if(user == ""){
            return res.json({Status:"Failed",Message:"No data Found", Data : {},Code:404});
           }
...
router.post('/delete', async function (req, res) {
      await waiter_billingModel.findByIdAndRemove(req.body._id, function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
           if(user == ""){
            return res.json({Status:"Failed",Message:"No data Found", Data : {},Code:404});
           }
